...
export class SubirFigura extends Component {
    ...
    handleChange=async e=>{
       await this.setState({
        ...
        && this.state.form.Altura.length < 3 && this.validarInput(this.state.form.Material) === true
        && this.validarInput(this.state.form.Marca) === true && this.state.form.Imagen != null  
        && this.validarInput(this.state.form.DescripcionImagen) === true && this.state.form.ExtensionImagen.length > 1)
        {
                    this.setState({
                        disabled: false
                    });
    ...
    async registrarFigura(e) {
        ...
        && this.validarInput(this.state.form.DescripcionImagen) === true && this.state.form.ExtensionImagen.length > 1)
        {
            ...
            fetch(API_LINK+"figuras/registrar", {
                method: "POST",
                body: figuraForm 
            })
            ...
            <div className="formGeneral">
                <form onSubmit={(e)=>this.registrarFigura(e)}>
                    ...
                    <div className="form-group">
                        ...
                        maxLength="20" name="DescripcionImagen"  onChange={this.handleChange} required/>
                    </div>
